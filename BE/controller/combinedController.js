...
exports.createClient = async (req, res, next) => {
    ...
    if (isClient) {
        try {
            ...
            const token = new Token({
                userId: client._id,
                token: 'client' + crypto.randomBytes(8).toString('hex') + '_client',
            });
            await token.save();
            console.log(token)

            const emailSubject = 'Account Details';
            const clientLoginLink = `http://localhost:3000/combined/login?workspace_name=${combinedWorkSpaceName}`;
            // const emailText = `Your account has been created.\n\nUsername: ${email}\nPassword: ${body.password}\nWorkspace : ${combinedWorkSpaceName}\nCreatedAt : ${client.createdAt}\nTo Login : ${clientLoginLink}`;
            ...
            await sendEmail(email, emailSubject, clientPassword, combinedWorkSpaceName, createdat, clientLoginLink);
